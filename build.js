...
function build(){
    ...
    var appServiceHtmlPath = path.join(__dirname, 'src', 'md-date-range-picker.service.html');
    ...
    var htmlServiceTemplate = fs.readFileSync(appServiceHtmlPath, 'utf8');
    var cssTemplate = fs.readFileSync(appCssPath, 'utf8');

    var css = new CleanCSS().minify(cssTemplate);

    var tmpjs = jsTemplate.replace('./md-date-range-picker.html', htm(htmlTemplate, htmlMinOpt).replace(/\'/g, '\\\'')).replace('templateUrl:','template:');
    var appjs = tmpjs.replace('./md-date-range-picker.service.html', htm(htmlServiceTemplate, htmlMinOpt).replace(/\'/g, '\\\'')).replace('templateUrl:','template:');
